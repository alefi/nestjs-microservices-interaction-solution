...
import { walletServiceGrpcServerOptions } from './config';

async function main() {
  const app = await NestFactory.createMicroservice<MicroserviceOptions>(AppModule, walletServiceGrpcServerOptions);
